...
const fieldLabels: Record<string, string> = {
  trip_id: "viagem",
  spent_at: "data",
  paid_by_person: "pessoa que pagou",
  ...
  split_pedro_percent: "% Pedro",
  split_camilly_percent: "% Camilly"
};
...
export function ExpenseForm({
  ...
  async function submit(values: ExpenseFormValues) {
    ...
    try {
      ...
      setSubmitError(submitErrorMessage(err, "Não foi possível salvar o gasto."));
    }
  }

  function submitInvalid(formErrors: FieldErrors<ExpenseFormInput>) {
  ...
  return (
    <View style={styles.form}>
      ...
      <Controller control={form.control} name="spent_at" render={({ field }) => <DateInput label="Data" value={field.value} onChangeText={field.onChange} error={errors.spent_at?.message} required />} />
      <View style={styles.grid}>
        <Controller control={form.control} name="paid_by_person" render={({ field }) => <Select label="Pessoa que pagou" value={field.value} onChange={field.onChange} options={personOptions} error={errors.paid_by_person?.message} required />} />
      ...
      <Controller control={form.control} name="amount" render={({ field }) => <MoneyInput label="Valor" value={String(field.value ?? "")} onChangeText={field.onChange} error={errors.amount?.message} required />} />
      ...
      <View style={styles.grid}>
        <Controller control={form.control} name="split_pedro_percent" render={({ field }) => <Input label="% Pedro" value={String(field.value ?? "")} onChangeText={field.onChange} keyboardType="decimal-pad" error={errors.split_pedro_percent?.message} required />} />
        <Controller control={form.control} name="split_camilly_percent" render={({ field }) => <Input label="% Camilly" value={String(field.value ?? "")} onChangeText={field.onChange} keyboardType="decimal-pad" error={errors.split_camilly_percent?.message} required />} />
      </View>
      ...
      <Button title="Salvar gasto" loading={loading || form.formState.isSubmitting} onPress={form.handleSubmit(submit, submitInvalid)} />
    </View>
  );
}
